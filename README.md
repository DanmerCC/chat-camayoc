...
- **Code Interpreter workflows:** Code and shell tools can run in the background, sandbox images return as viewable artifacts, and highly experimental stateful sessions use dedicated routing, admin-restricted workspace scopes, and a personal default for new Agents.
- **Agent extensibility:** Experimental Agent Plugins can bundle deployment Skills, MCP servers, and opt-in command hooks; subagents can run saved Agent teams and preserve navigable, view-only child threads.
- **Agent automation foundation:** Trusted schedules, webhooks, queues, MCP adapters, and internal events can share durable, source-neutral fire and steer delivery.
