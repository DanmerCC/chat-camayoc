...
export default function useReplyWatcher() {
  ...
  const observedAtRef = useRef<Map<string, number>>(new Map());
  ...
  useEffect(() => {
    ...
    for (const conversationId of running) {
      observedAtRef.current.set(conversationId, observedAt);
    }
    ...
    for (const conversationId of previous) {
      ...
      const lastObservedAt = observedAtRef.current.get(conversationId) ?? 0;
      observedAtRef.current.delete(conversationId);
      /* The tab holding the stream stamps the list cache in its final handler; when that
         stamp is newer than the last time this tab saw the job running, the fetch is
         redundant. */
